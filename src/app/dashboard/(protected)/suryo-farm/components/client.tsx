...
import { Thermometer, Droplets, FlaskConical, Zap, Power, Bot, User, ChevronsDown, ChevronsUp, Waves } from 'lucide-react';
...
const MODE_STATUS_TOPIC = 'hidroponicKloning/control/mode/status';
const MODE_COMMAND_TOPIC = 'hidroponicKloning/control/mode/command';

const actuatorTopics = {
  nutrisi: { command: 'hidroponicKloning/actuator/nutrisi/command', status: 'hidroponicKloning/actuator/nutrisi/status' },
  phUp: { command: 'hidroponicKloning/actuator/phUp/command', status: 'hidroponicKloning/actuator/phUp/status' },
  phDown: { command: 'hidroponicKloning/actuator/phDown/command', status: 'hidroponicKloning/actuator/phDown/status' },
  pump: { command: 'hidroponicKloning/actuator/pump/command', status: 'hidroponicKloning/actuator/pump/status' },
};
    ...
    <>
      ...
      <div className={`bg-white p-6 mb-6 rounded-lg shadow-md transition-opacity ${!isManualMode || isGuest ? 'opacity-60 cursor-not-allowed' : ''}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <ActuatorButton label="pH Up" icon={ChevronsUp} status={pompaPhUpStatus} onToggle={() => handleActuatorToggle('phUp', pompaPhUpStatus)} disabled={!isManualMode || isGuest} color="sky" />
