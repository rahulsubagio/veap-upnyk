...
const SettingsClient = () => {
  ...
  const [humiThreshold, setHumiThreshold] = useState('');
  ...
  // --- Topik MQTT ---
  const SETTINGS_COMMAND_TOPIC = 'gh/control/settings/command';
  const CALIBRATE_COMMAND_TOPIC = 'gh/ph/calibrate/command';
  const SENSOR_DATA_TOPIC = 'gh/sensors/data';
  const SETTINGS_STATUS_TOPIC = 'gh/control/settings/status';
  const SETTINGS_GET_TOPIC = 'gh/control/settings/get';
    ...
    mqttClient.on('message', (topic, message) => {
      ...
      if (topic === SENSOR_DATA_TOPIC) {
        ...
        try {
          ...
          setTempThreshold(settings.temp_threshold.toString());
          setHumiThreshold(settings.humi_threshold.toString());
  ...
  const handleSaveSettings = () => {
    if (client && client.connected) {
      ...
      const settingsPayload = {
        temp_threshold: parseInt(tempThreshold, 10),
        humi_threshold: parseFloat(humiThreshold),
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                  <label htmlFor="lowph" className="block text-sm font-medium text-gray-700 mb-2">Ambang Batas Suhu Udara</label>
                  <input type="number" step="0.1" id="lowph" value={tempThreshold} onChange={(e) => setTempThreshold(e.target.value)} className="w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 text-gray-900 focus:ring-cyan-500 focus:border-cyan-500" />
                  <p className="text-xs text-gray-500 mt-1">Kipas Exhaust akan nyala.</p>
              </div>
              <div>
                  <label htmlFor="highph" className="block text-sm font-medium text-gray-700 mb-2">Ambang Batas Kelembapan</label>
                  <input type="number" step="0.1" id="highph" value={humiThreshold} onChange={(e) => setHumiThreshold(e.target.value)} className="w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 text-gray-900 focus:ring-cyan-500 focus:border-cyan-500" />
                  <p className="text-xs text-gray-500 mt-1">Selenoid akan nyala.</p>
              </div>
            </div>
